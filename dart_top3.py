...
function renderVerificationEntry(e) {
  ...
  const sign = e.change_amount > 0 ? '+' : '';
  return `
    <div class="card verify-item">
      <div class="card-head">
        <div class="corp">
          <p class="corp-name">${e.corp_name}</p>
          <div class="verify-meta">${e.rec_date} 추천 · 전일종가 ${e.prev_close.toLocaleString()} · 당일고가 ${e.day_high != null ? e.day_high.toLocaleString() : '-'}</div>
        </div>
        <span class="delta delta-total ${dirClass}">${dirArrow} ${sign}${e.change_pct}% (${sign}${e.change_amount.toLocaleString()})</span>
      </div>
    </div>`;
}
